...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-card border border-border shadow-card overflow-hidden">
            <div class="px-5 py-4 border-b border-border bg-primary/5">
                <h2 class="text-lg font-semibold text-slate-800">Assigned Events</h2>
            </div>
            <div class="p-5">
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="bg-slate-100 border-b border-border">
                                    <th class="text-left text-xs font-semibold text-slate-600 uppercase tracking-wider px-4 py-2">Event Name</th>
                                ...
                                @foreach($vendor->events as $event)
                                    <tr class="border-b border-border odd:bg-slate-50/50 hover:bg-primary/5">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <form action="{{ route('college.vendors.remove-from-event', [$vendor, $event]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to remove this vendor from the event?');">
                                                @csrf
                                                @method('DELETE')
                ...
                @else
                    <p class="text-slate-500 text-sm">No events assigned.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div>
        <div class="bg-white rounded-card border border-border shadow-card overflow-hidden sticky top-24">
            <div class="px-5 py-4 border-b border-border bg-primary/5">
                <h2 class="text-lg font-semibold text-slate-800">Assign to Event</h2>
            </div>
            <div class="p-5">
                <form action="{{ route('college.vendors.assign-event', $vendor) }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="event_id" class="block text-sm font-medium text-slate-700 mb-1">Event <span class="text-red-500">*</span></label>
                    ...
                    <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 rounded-button font-medium text-white bg-primary hover:bg-primary-hover">Assign to Event</button>
                </form>
            </div>
        </div>
    </div>
