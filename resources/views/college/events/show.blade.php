...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-primary/10 rounded-card border border-primary/20 shadow-card p-5">
        <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">Programs</p>
...
<div class="space-y-6">
    <div class="bg-white rounded-card border border-border shadow-card overflow-hidden">
        ...
        <div class="p-5">
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 text-sm">
                <dt class="font-medium text-slate-500">Name:</dt><dd class="text-slate-800">{{ $event->name }}</dd>
                @if($event->description)<dt class="font-medium text-slate-500">Description:</dt><dd class="text-slate-800">{{ $event->description }}</dd>@endif
                @if($event->target_audience)<dt class="font-medium text-slate-500">Target Audience:</dt><dd class="text-slate-800">{{ $event->target_audience }}</dd>@endif
                <dt class="font-medium text-slate-500">Start Date:</dt><dd class="text-slate-800">{{ $event->start_date->format('M d, Y') }}</dd>
                <dt class="font-medium text-slate-500">End Date:</dt><dd class="text-slate-800">{{ $event->end_date->format('M d, Y') }}</dd>
    ...
    <div class="bg-white rounded-card border border-border shadow-card overflow-hidden">
        <div class="px-5 py-4 border-b border-border bg-primary/5 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-800">Programs</h2>
        ...
        <div class="overflow-x-auto">
            ...
            @else
                <div class="p-8 text-center text-slate-500 text-sm">
                    No programs yet. <a href="{{ route('college.events.programs.create', $event) }}" class="text-primary font-medium hover:underline">Add a program</a> and assign who runs it (Vendor or Independent Trainer) and an Internal Program Manager.
                </div>
            @endif
        </div>
    </div>
