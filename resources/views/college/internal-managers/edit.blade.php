        ...
        <form action="{{ route('college.internal-managers.update', $internalManager) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="department_id" required class="w-full rounded-input border border-slate-300 focus:ring-2 focus:ring-primary focus:border-primary bg-white @error('department_id') border-red-500 @enderror">
                    <option value="" disabled {{ old('department_id', $internalManager->department_id) ? '' : 'selected' }}>Select department</option>
                    @forelse($departments as $d)
                        <option value="{{ $d->id }}" {{ (string) old('department_id', $internalManager->department_id) === (string) $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                    @empty
