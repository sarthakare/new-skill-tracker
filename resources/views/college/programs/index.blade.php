...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="flex items-center gap-2 text-2xl font-semibold text-slate-800">
            ...
            Subjects/programs for {{ $event->name }}
        </h1>
        <p class="mt-1 text-sm text-slate-500">Manage subjects/programs and manager assignments.</p>
    </div>
...
<div class="bg-white rounded-card border border-border shadow-card overflow-hidden">
    <div class="px-5 py-4 border-b border-border bg-primary/5 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-slate-800">All Subjects/programs</h2>
        <span class="text-sm text-slate-600">{{ $programs->total() }} {{ $programs->total() === 1 ? 'subject/program' : 'subjects/programs' }}</span>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full min-w-[800px]">
            ...
            <tbody>
                    ...
                    <tr class="border-b border-border odd:bg-slate-50/50 hover:bg-primary/5 transition-colors">
                        ...
                        <td class="px-5 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form action="{{ route('college.events.programs.destroy', [$event, $program]) }}" method="POST" class="inline" onsubmit="return confirm('Delete this subject/program?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-5 py-12 text-center text-slate-500">No subjects/programs created yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
